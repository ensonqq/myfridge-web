<template>
  <v-container>
    <v-row>
      <v-col>
        <h1>{{ $t('termsAndConditions') }}</h1>
        <template v-if="$i18n.locale === 'zh'">
          <h3 class="mx-1">生效時間：2017年 1 月 1 日</h3>
          <p> myfridge 的使命在於為香港人製造健康及高蛋白食品。我們致力協助您在增肌，修身，減肥，健康飲食中增加優質蛋白的攝取。</p>
          <p>我們堅持每日新鮮生產，香港製造，讓本地香港人能快速，安全地享用我們的產品。透過穩定的低溫門到門送貨服務，讓忙碌的你無論在任何時侯及地點都可以攝取最優質蛋白質。</p>
          <p>我們明白一個嚴格的產品製造過程和良好的送貨服務能讓用戶得到最佳體驗，所以我們會一直朝著這個方向堅持，承諾為客戶提供最好產品及服務。</p>

          <h3 class="mx-1">1. 前言</h3>
          <h4>1.1 守則及條款</h4>
          <p>使用 myfridge 的「服務」即表示您同意所有守則及條款。使用 myfridge 的「服務」也必須遵守隱私政策，後者的內容規範了 myfridge 收集、使用、分享以及儲存您的個人資訊時所採行的方式。</p>
          <p>您同意一旦在 myfridge 開立戶口（不論經由網站或是應用程式）或經任何途徑使用我們的「服務」，即表示您同意本守則及條款中的所有項目（我們會跟據實際情況不定時更新其中任何的守則或條款）。如果您不同意本守則及條款中的任何項目，請勿在 myfridge 開立戶口並不要經任何途徑使用我們的「服務」。如果您要終止使用 myfridge 的「服務」，您可以透過電郵或電話向我們提出。</p>
          <p>本守則及條款中的「服務」指由 myfridge 的應用程式或網站或其他溝通渠道所提供的任何功能、資訊或產品，當中包括電郵、產品、輸送服務、訂單及優惠等。</p>
          <p> myfridge 由 滿記凍肉有限公司 全資擁有及全權營運（在本私隱政策聲明中，各自稱為「 myfridge 」、「我們」或「本公司」，視適用情況而定）。</p>
          <p>本守則及條款適用於會員及訪客。</p>
          <p>作為 myfridge 「服務」的「訪客」或「會員」，您個人資料的收集、使用及分享必須遵守隱私政策及其更新。</p>

          <h4>1.2 會員與訪客</h4>
          <p>當您透過任何途經於 myfridge 註冊帳戶，您即為會員。若您選擇不註冊使用我們的「服務」，也能以訪客身份使用某些功能。</p>
          <h4>1.3 變更</h4>
          <p>基於商業發展考慮，我們可能會不時變更本守則及條款中的內容。</p>
          <p>我們隨時可能修改本守則及條款及「隱私政策」。如有重大變更，我們會透過「服務」或其他方式通知您，讓您有機會在變更生效前詳閱變更內容。我們同意變更不能溯及既往。若您不同意任何變更後的內容，您可關閉帳戶。在我們公布或傳送條款變更通知後，若您繼續使用我們的「服務」，即代表您同意自生效日起遵循更新的條款。</p>
          <h3 class="mx-1">2. 義務</h3>
          <h4>2.1. 服務適用性</h4>
          <p> myfridge 的「服務」不適合 16 歲以下的任何人士使用。</p>

          <h4>2.2 您的帳戶</h4>
          <p>您將對密碼保密。</p>
          <p>您將不會與他人共用帳戶，並將遵守所有於您適用的法律及我們的規定。</p>
          <p>會員是帳戶持有人。您同意：(1) 使用安全度高的密碼且不隨意對外透露；(2) 不會轉讓帳戶的任一部分（如聯絡人）。除非您關閉帳戶或檢舉濫用情事，否則您的所有帳戶活動由您全權負責。</p>

          <h4>2.3. 付款</h4>
          <p>當您購買我們的「服務」時，您將會履行付款義務。我們不會儲存您的任何付款資料（例如 信用咭資料）。您明白我們的付款程序是交由我們選用的第三方服務商進行，因此我們並不對付款過程中的任何不當使用或處理資料而負任何責任。</p>
          <p>我們將不定時提供折扣而您可以於付費時輸入我們提供的折扣碼以取得折扣價。</p>
          <p>我們的「服務」不設退款，除非是因為我們的技術問題導致您不能正常使用已購買的「服務」而我們不能透過其他渠道為您提供「服務」。</p>
          <p>若您購買我們的任何一項付費「服務」，表示您同意支付相關手續費和稅金如果適用，如未付款，我們會終止付費「服務」。</p>
          <p>您可能因所在地點的緣故而需要支付匯兌手續費或匯差（例如匯率）。</p>
          <p>您可以在帳戶資料登記的電郵中收到發票副本。</p>

          <h4>2.4. 通知及訊息</h4>
          <p>您同意我們透過網站、應用程式和聯絡資訊提供通知和訊息給您。如果您的聯絡資訊已過時，您可能會因此錯過重要通知。</p>

          <h4>2.5 分享</h4>
          當您分享在我們的「服務」上獲取的資訊時，您同意您所分享的資訊符合所有適用於您的法律及法規，我們不會就您的分享行為而所產生的第三者申索或後果而負責。

          <h3 class="mx-1">3. 權利和限制</h3>
          <h4>3.1. 您授予我們的權限</h4>
          <p>您擁有所有您提供的內容、訂單記錄及個人資訊，但您也授予我們非獨佔性的相關權限去使用這些資訊。</p>
          <p>我們會在我們其他的相關服務中使用您提供的內容、產品喜好及個人資訊。（例如推送您最合適的產品）</p>
          <p>按照使用條款，您傳送或發表於「服務」的內容及資訊歸您所有，您只授予我們和我們的關聯機構（關聯機構是指受我們控制、我們受其控制或與我們彼此共同控制之實體）下列非獨佔性授權：</p>
          <p>具全球性、可轉讓且可轉授權的權利，包括使用、複製、修改、散佈、發表以及處理您透過我們的「服務」所提供的資訊與內容，不需進一步徵得您的同意、不需通知及／或提供補償。這些權利受到以下方式所限制：</p>
          <ol>
            <li>已經透過使用我們的「服務」傳送給我們的資訊。</li>
            <li>未經您同意，我們不會在提供給他人的產品／平台／廣告或第三方服務中包含該內容（包括贊助內容）。</li>
            <li>我們可能會編輯或移除您所提交的訂單內容（如：貨品短缺、下架等）。</li>
          </ol>
          <p>您和我們同意，如果內容包含個人資料，則必須遵守我們的隱私政策。</p>
          <p>您和我們同意我們可按照隱私政策存取、儲存、處理和使用您提供的任何資訊和個人資料。</p>
          <p>傳送與我們的「服務」相關的建議或其他意見反映，即表示您同意我們不需支付任何補償，便可將您的意見使用及分享（但非必要）到任何用途。</p>
          <p>請確認您僅提供您有權分享的資訊和內容，並確認您的 myfridge 檔案資料屬實。</p>
          <p>您同意只提供不違法也不違反他人權利（包括智慧財產權）的內容或資訊。您也同意您的個人檔案資訊屬實。為符合當地法令， myfridge 在某些國家可能需要移除部分內容。</p>

          <h4>3.2. 服務可用性</h4>
          <p>我們在未來可能收費或終止任何「服務」或修改價格。</p>
          <p>我們可能變更、停用或中止任何我們的「服務」。我們也可能修改價格，並在法律許可的合理通知期間後於未來生效。</p>
          <p>我們不保證會儲存或持續顯示您發表的任何資訊及內容。 myfridge 非儲存服務。除適用法律規定及 我們的「隱私政策」所規定的範圍以外，您同意我們沒有義務儲存、保留或提供您或他人所提供之任何內容或資訊的副本給您。</p>

          <h4>3.3 限制</h4>
          <p>我們有權於「服務」限制您建立關係及互動的方式。</p>

          <h4>3.4 智慧財產權</h4>
          <p>我們會提供與 myfridge 的智慧財產權相關的通知。</p>
          <p>我們保留「服務」的所有智慧財產權。「服務」所用之商標及標誌可能為其個別所有者之商標。 myfridge 及其標誌和其他相關商標、服務圖標、圖片，以及「服務」所用的標誌皆為我們的商標或註冊商標。</p>


          <h3 class="mx-1">4. 免責聲明和責任限制</h3>
          <h4>4.1. 無擔保</h4>
          <p>我們不負任何「服務」之品質、安全或可信度之法律責任。</p>
          <p>我們及關聯機構不代表或擔保任何這類服務，包含此類服務將不會中斷或零錯誤的任何陳述，並依「現況」和「現有」基礎提供此類服務（包含內容與資訊）。在適用法律允許之所有範圍內，我們與關聯機構不對任何默示或法定的擔保負責，包括任何權利、資料正確性、不侵權、適售性或符合特定目的之默示擔保。</p>

          <h4>4.2. 責任免除</h4>
          <p>以下為所有我們可能擁有的法律責任限制。</p>
          <p>在法律允許之所有範圍內（且除非 我們另外簽訂取代此守則及條款的協議），我們包括關聯機構對於因本守則及條款而發生的利潤損失或商業機會損失、名譽損失（例如具冒犯性或誹謗性的言論）、資料損失（例如停機時間或損失、使用或變更資訊或內容），或任何間接的、附帶的、衍生的、特別的或懲罰性的損害概不負責。</p>

          <h4>4.3 協議依據；例外條款</h4>
          第 4 節中所述的責任限制為您和 我們 協議的依據，並適用於所有責任賠償（如擔保、侵權行為、過失、合約、法律），不論 我們或關聯機構是否已得知發生此類損害的可能性，也不論這些補救措施能否達成必要的目的。

          <h3 class="mx-1">5. 準據法和爭議解決</h3>
          儘管十分罕見，但倘若在法律方面出現爭議，您與我們均同意在中國香港特別行政區法院（以香港特別行政區法律為準據）解決。
          本節條款並不會剝奪您居留國家法律規範中，對消費者保障的任何強制約束力，在我們提供給您的「服務」中，您仍可享有這些保障。

          <h3 class="mx-1">6. 一般條款</h3>
          <p>以下為關於本守則及條款的一些重要細則。</p>
          <p>如果對本守則及條款具管轄權的法院認為本守則及條款有任何無法履行的部分，雙方均同意法院應以保留原始目的又能履行的方式修改條款。如果法院無法履行，雙方同意要求法院移除無法履行的部分，並履行守則及條款中的其餘條款。</p>
          <p>本守則及條款（包括其他您使用「服務」時我們所提供的條款）為與服務相關並用於約束雙方的唯一條款，同時取代所有較早的「服務」協議。</p>
          <p>如果我們對違反守則及條款行為未採取法律行動，並不代表我們放棄履行的權利。</p>

          <h3 class="mx-1">7. Optmeal 「能做」與「不能做」</h3>
          <h4>7.1. 能做</h4>
          <p> Optmeal 是一個健康優質蛋白產品銷售平台。這個清單以規範了您在使用我們的「服務」時可以和不可以做的事情。</p>
          <p>您同意您將：</p>
          <ol>
            <li>遵守所有適用法律，包括但不僅限於隱私權法、智慧財產權法、反垃圾訊息法、出口管理條例、稅務法及監管要求；</li>
            <li>提供正確資訊並及時更新；</li>
            <li>個人檔案中使用真實姓名；並且</li>
            <li>以正確方式使用「服務」，特別在進行購買活動時。</li>
          </ol>

          <h4>7.2 不能做</h4>
          <p>您同意您將不會：</p>
          <ol>
            <li>在 Optmeal 上建立虛假身份、謊報身份、建立非您本人（非真人）之會員個人檔案，或是使用或試圖使用他人帳戶；</li>
            <li>開發、支援或使用軟體、裝置、指令碼、自動程式或其他方式或程序（包括網路爬蟲、瀏覽器外掛程式和附加元件，或任何其他技術）來抓取「服務」或以其他方式從「服務」複製個人檔案和其他資料；</li>
            <li>覆寫任何安全功能，或是迴避或規避任何「服務」的存取控制或使用限制（如以大寫字搜尋關鍵字或瀏覽個人檔案）；</li>
            <li>未經 我們同意而複製、使用、披露或散佈任何從「服務」取得之資訊（不論是以直接或經由第三方（如搜尋引擎）的間接方式）；</li>
            <li>披露您未經同意即披露的資訊（如他人的機密資訊）；</li>
            <li>違反他人智慧財產權，包括：著作權、專利、商標、商業機密或其他專利權。</li>
            <li>違反 我們的智慧財產權或其他權利，包括但不限於 (i) 複製或散佈我們的學習影片或其他資料，或 (ii) 複製或散佈我們的技術，依開放資源授權發佈的技術則不在此限；(iii) 在任何商業名稱、電子郵件或網址使用「 Optmeal 」字眼或標誌；</li>
            <li>反向工程、反編譯、解譯、破解或以其他方式試圖獲取「服務」或非開放原始碼之任何相關技術的原始程式碼；</li>
            <li>未經我們同意，默示或明示您為 Optmeal 附屬個體或獲得 Optmeal 背書（如：表示您為 Optmeal 認可的顧問）；</li>
            <li>未經我們同意即出租、抵押、交易、販售／轉售「服務」或相關資料或其存取權，或以其他方式利用「服務」或相關資料或其存取權賺取收益；</li>
            <li>使用自動程式或其他自動方法使用「服務」、新增或下載聯絡人、傳送或重新導向訊息；</li>
            <li>為任何競爭目的，監視「服務」的可用性、表現或功能；</li>
            <li>進行「架構」、「鏡像」或其他模擬「服務」外觀或功能之行為；</li>
            <li>覆蓋或以其他方式修改「服務」或其外觀（例如透過插入元素到「服務」，或移除、遮蓋或模糊「服務」所含之廣告）；</li>
            <li>干擾「服務」的運作或製造不合理的負載（如：垃圾郵件、阻斷服務攻擊、病毒、遊戲演算）；及／或</li>
          </ol>

          <h3 class="mx-1">8. 內容相關的申訴</h3>
          <p>申訴聯絡資訊列於以下項目。</p>

          <h3 class="mx-1">9. 如何聯絡我們</h3>
          <div class="mx-1">電郵︰info@Optmeal.com</div>
          <div class="mx-1">電話︰65657682</div>
        </template>
        <template v-else>
          <h3 class="mx-1">Effective time: January 1, 2017</h3>
          <p> Optmeal's mission is to create healthy and high-protein foods for Hong Kong people. We are committed to helping you gain muscle, slim down, lose weight, and increase your intake of high-quality protein in a healthy diet. </p>
          <p>We insist on fresh production every day and made in Hong Kong, so that local Hong Kong people can enjoy our products quickly and safely. Through stable low-temperature door-to-door delivery service, busy people can consume the highest quality protein at any time and place. </p>
          <p>We understand that a strict product manufacturing process and good delivery service can provide users with the best experience, so we will always adhere to this direction and promise to provide customers with the best products and services. </p>

          <h3 class="mx-1">1. Preface</h3>
          <h4>1.1 Code and Terms</h4>
          <p>By using Optmeal's Services, you agree to all Terms and Conditions. Use of Optmeal's "Services" is also subject to the Privacy Policy, which governs how Optmeal collects, uses, shares and stores your personal information. </p>
          <p>You agree that once you open an account at Optmeal (whether through the website or application) or use our "Services" through any means, you agree to all items in this Code and Terms (we will act accordingly based on the actual situation). Any of these rules or terms may be updated from time to time). If you do not agree with anything in these Terms and Conditions, please do not open an account with Optmeal and do not use our Services through any means. If you want to terminate your use of Optmeal's "Services", you can notify us by email or phone. </p>
          <p>The "services" in these Terms and Conditions refer to any functions, information or products provided by Optmeal's applications or websites or other communication channels, including emails, products, delivery services, orders and offers, etc. </p>
          <p> Optmeal is wholly owned and operated by 滿記凍肉有限公司 (in this privacy policy statement, each is referred to as "Optmeal", "we" or "the Company", as applicable depending on the situation). </p>
          <p>These Terms and Conditions apply to members and visitors. </p>
          <p>As a "Visitor" or "Member" of the Optmeal "Service", the collection, use and sharing of your personal information must comply with the Privacy Policy and its updates. </p>

          <h4>1.2 Members and Visitors</h4>
          <p>When you register an account with Optmeal through any means, you become a member. If you choose not to register to use our Services, you may also be able to use certain features as a guest. </p>
          <h4>1.3 Changes</h4>
          <p>Based on business development considerations, we may change the contents of this Code and Terms from time to time. </p>
          <p>We may modify these Terms and Conditions and the Privacy Policy at any time. If there are major changes, we will notify you through the "Service" or other means to give you the opportunity to read the changes in detail before they take effect. We agree that changes cannot be retroactive. If you do not agree with any changed content, you may close your account. If you continue to use our "Services" after we publish or transmit a notice of changes to the terms, you agree to abide by the updated terms from the effective date. </p>
          <h3 class="mx-1">2. Obligations</h3>
          <h4>2.1. Service applicability</h4>
          <p> Optmeal's Services are not intended for use by anyone under the age of 16. </p>

          <h4>2.2 Your Account</h4>
          <p>You will keep your password confidential. </p>
          <p>You will not share your account with others and will comply with all laws that apply to you and our rules. </p>
          <p>Members are account holders. You agree to: (1) use a strong password and not disclose it to others; (2) not transfer any part of your account (such as contacts). Unless you close your account or report abuse, you are solely responsible for all activity on your account. </p>

          <h4>2.3. Payment</h4>
          <p>When you purchase our Services, you will be subject to payment obligations. We do not store any of your payment information (such as credit card information). You understand that our payment process is performed by a third-party service provider selected by us, and therefore we are not responsible for any improper use or processing of data during the payment process. </p>
          <p>We will provide discounts from time to time and you can enter the discount code we provide when paying to get the discounted price. </p>
          <p>Our "Services" are non-refundable, unless it is due to our technical problems that you cannot use the purchased "Services" normally and we are unable to provide you with "Services" through other channels. </p>
          <p>If you purchase any of our paid "services", you agree to pay the relevant handling fees and taxes, if applicable. If payment is not made, we will terminate the paid "services". </p>
          <p>Depending on your location, you may be charged exchange fees or differences (such as exchange rates). </p>
          <p>You can receive a copy of the invoice in the email registered in your account information. </p>

          <h4>2.4. Notifications and Messages</h4>
          <p>You agree that we may provide you with notifications and information through our website, applications and contact information. If your contact information is out of date, you may miss important notifications. </p>

          <h4>2.5 Share</h4>
          When you share information obtained on our "Services", you agree that the information you share complies with all laws and regulations that apply to you, and we will not have any third-party claims or claims arising from your sharing behavior. responsible for the consequences.

          <h3 class="mx-1">3. Rights and Restrictions</h3>
          <h4>3.1. Permissions You Grant Us</h4>
          <p>You own all content, order records, and personal information you provide, but you also grant us non-exclusive permission to use this information. </p>
          <p>We will use the content, product preferences and personal information you provide in our other related services. (For example, push your most suitable products)</p>
          <p>According to the Terms of Use, the content and information you transmit or publish on the "Service" belongs to you, and you only grant us and our affiliates (affiliates are those controlled by us, controlled by us, or jointly with us) Controlled Entity) the following non-exclusive license:</p>
          <p>The worldwide, transferable and sublicensable right to use, copy, modify, distribute, publish and process the information and content you provide through our Services without further obtaining your consent. Agree, without notification and/or compensation. These rights are limited in the following ways:</p>
          <ol>
            <li>Information that has been sent to us through the use of our Services.</li>
            <li>We will not include this content (including sponsored content) in products/platforms/advertising or third-party services provided to others without your consent.</li>
            <li>We may edit or remove the order content you submitted (such as product shortage, removal from shelves, etc.).</li>
          </ol>
          <p>You and we agree that if content contains personal data, it must comply with our privacy policy. </p>
          <p>You and we agree that we may access, store, process and use any information and personal data you provide in accordance with the Privacy Policy. </p>
          <p>By sending suggestions or other feedback related to our Services, you agree that we may use and share your comments (but not necessarily) for any purpose without paying any compensation. </p>
          <p>Please make sure you only provide information and content that you are authorized to share, and that the information in your Optmeal profile is accurate. </p>
          <p>You agree to only provide content or information that is not illegal and does not violate the rights of others, including intellectual property rights. You also agree that your profile information is true. Optmeal may need to remove some content in some countries to comply with local laws. </p>

          <h4>3.2. Service availability</h4>
          <p>We may charge fees or discontinue any Services or modify prices in the future. </p>
          <p>We may change, discontinue or discontinue any of our Services. We may also revise prices with effect in the future after reasonable notice period as permitted by law. </p>
          <p>We do not guarantee that any information and content you post will be stored or continuously displayed. Optmeal non-storage service. Except to the extent required by applicable law and our Privacy Policy, you agree that we have no obligation to store, retain or provide you with copies of any content or information provided by you or others. </p>

          <h4>3.3 Limitations</h4>
          <p>We reserve the right to limit how you establish relationships and interact with us on the Service. </p>

          <h4>3.4 Intellectual property rights</h4>
          <p>We will provide notices regarding Optmeal's intellectual property rights. </p>
          <p>We retain all intellectual property rights in the Service. Trademarks and logos used in the Service may be the trademarks of their respective owners. Optmeal, its logo and other related trademarks, service icons, images, and logos used in the Service are our trademarks or registered trademarks. </p>


          <h3 class="mx-1">4. Disclaimer and Limitation of Liability</h3>
          <h4>4.1. No guarantee</h4>
          <p>We are not legally responsible for the quality, safety or reliability of any Service. </p>
          <p>We and our affiliates do not represent or guarantee any such services, including any representation that such services will be uninterrupted or error-free, and provide such services (including content) on an "as is" and "as available" basis. and information). To the full extent permitted by applicable law, we and our affiliates disclaim any implied or statutory warranties, including any implied warranties of title, accuracy of information, non-infringement, merchantability or fitness for a particular purpose. </p>

          <h4>4.2. Disclaimer</h4>
          <p>The following are any limitations on any legal liability we may have. </p>
          <p>To the full extent permitted by law (and unless we enter into another agreement that replaces these Codes and Terms), we, including our affiliates, are not responsible for any loss of profits or business opportunities or reputational damage (such as specific Offensive or defamatory statements), loss of data (such as downtime or loss of use or alteration of information or content), or any indirect, incidental, consequential, special or punitive damages. </p>

          <h4>4.3 Basis of Agreement; Exceptions</h4>
          The limitations of liability set forth in Section 4 are the basis of the agreement between you and us and apply to all claims of liability (such as warranty, tort, negligence, contract, law) whether or not we or an affiliate were informed of the occurrence of such damages possibility, regardless of whether these remedial measures achieve the necessary purpose.

          <h3 class="mx-1">5. Governing law and dispute resolution</h3>
          Although rare, if a legal dispute arises, you and we agree to resolve it in the courts of the Hong Kong Special Administrative Region, China (subject to the laws of the Hong Kong Special Administrative Region).
          The terms of this section will not deprive you of any mandatory binding consumer protections under the laws and regulations of your country of residence. You can still enjoy these protections in the "services" we provide to you.

          <h3 class="mx-1">6. General Terms</h3>
          <p>The following are some important details about this Code and Terms. </p>
          <p>If a court with jurisdiction over these Codes and Terms finds that any part of these Codes and Terms is unenforceable, both parties agree that the court should modify the terms in a way that retains the original purpose and can be performed. If the court is unable to perform, the parties agree to ask the court to remove the unenforceable portion and fulfill the remaining provisions of the Code and Terms. </p>
          <p>This Code and Terms (including other terms we provide when you use the "Service") are the only terms related to the Service and binding on both parties, and supersede all earlier "Service" agreements. </p>
          <p>If we do not take legal action for violations of the Code and Terms, it does not mean that we have waived our right to perform. </p>

          <h3 class="mx-1">7. What Optmeal can and cannot do</h3>
          <h4>7.1. Can do</h4>
          <p> Optmeal is a sales platform for healthy and high-quality protein products. This list regulates what you can and cannot do when using our Services. </p>
          <p>You agree that you will:</p>
          <ol>
            <li>Comply with all applicable laws, including but not limited to privacy laws, intellectual property laws, anti-spam laws, export administration regulations, tax laws, and regulatory requirements;</li>
            <li>Provide correct information and update it in a timely manner;</li>
            <li>Use your real name on your profile; and</li>
            <li>Use the Service in the correct manner, especially when making purchases.</li>
          </ol>

          <h4>7.2 Can’t do</h4>
          <p>You agree that you will not:</p>
          <ol>
            <li>Create a false identity on Optmeal, misrepresent your identity, create a member profile that is not your own (not a real person), or use or attempt to use another person's account;</li>
            <li>Develop, support or use software, devices, scripts, automated programs or other means or processes (including web crawlers, browser plug-ins and add-ons, or any other technology) to crawl the Service or otherwise Methods to copy profiles and other information from the Service;</li>
            <li>Override any security features, or circumvent or circumvent any access controls or usage restrictions on the Service (such as searching for keywords in capital letters or browsing profiles);</li>
            <li>Copy, use, disclose or distribute any information obtained from the Service (whether directly or indirectly through a third party (such as a search engine)) without our consent;</li>
            <li>Disclose information that you disclosed without your consent (such as another person's confidential information);</li>
            <li>Violate the intellectual property rights of others, including copyrights, patents, trademarks, trade secrets or other proprietary rights.</li>
            <li>Violate our intellectual property rights or other rights, including but not limited to (i) copying or distributing our learning videos or other materials, or (ii) copying or distributing our technology, except for technology released under an open source license. This limit; (iii) use the word "Optmeal" or the logo in any business name, email or website;</li>
            <li>Reverse engineer, decompile, interpret, crack or otherwise attempt to obtain the source code of the Service or any related technology that is not open source;</li>
            <li>Imply or expressly imply that you are an Optmeal affiliate or that you are endorsed by Optmeal (e.g., indicating that you are an Optmeal-approved consultant) without our consent;</li>
            <li>Lease, mortgage, trade, sell/resell the Services or related information or access rights thereto without our consent, or otherwise use the Services or related information or access rights to earn income ;</li>
            <li>Use automated programs or other automated methods to use the Service, add or download contacts, send or redirect messages;</li>
            <li>Monitor the availability, performance or functionality of the Service for any competitive purposes;</li>
            <li>Carry out "framing", "mirroring" or other actions that simulate the appearance or functionality of the "Service";</li>
            <li>Overlay or otherwise modify the Service or its appearance (such as by inserting elements into the Service, or removing, obscuring or obscuring advertisements contained in the Service);</li>
            <li>Interfering with the operation of the Service or creating an unreasonable load (e.g. spam, denial of service attacks, viruses, game algorithms); and/or</li>
          </ol>

          <h3 class="mx-1">8. Content-related complaints</h3>
          <p>Complaint contact information is listed below. </p>

          <h3 class="mx-1">9. How to contact us</h3>
          <div class="mx-1">Email: info@Optmeal.com</div>
          <div class="mx-1">Tel/Whatsapp: 65657682</div>
        </template>
        <template v-else>
          <h3 class="mx-1">Effective time: January 1, 2017</h3>
          <p> Optmeal's mission is to create healthy and high-protein foods for Hong Kong people. We are committed to helping you gain muscle, slim down, lose weight, and increase your intake of high-quality protein in a healthy diet. </p>
          <p>We insist on fresh production every day and made in Hong Kong, so that local Hong Kong people can enjoy our products quickly and safely. Through stable low-temperature door-to-door delivery service, busy people can consume the highest quality protein at any time and place. </p>
          <p>We understand that a strict product manufacturing process and good delivery service can provide users with the best experience, so we will always adhere to this direction and promise to provide customers with the best products and services. </p>

          <h3 class="mx-1">1. Preface</h3>
          <h4>1.1 Code and Terms</h4>
          <p>By using Optmeal's Services, you agree to all Terms and Conditions. Use of Optmeal's "Services" is also subject to the Privacy Policy, which governs how Optmeal collects, uses, shares and stores your personal information. </p>
          <p>You agree that once you open an account at Optmeal (whether through the website or application) or use our "Services" through any means, you agree to all items in this Code and Terms (we will act accordingly based on the actual situation). Any of these rules or terms may be updated from time to time). If you do not agree with anything in these Terms and Conditions, please do not open an account with Optmeal and do not use our Services through any means. If you want to terminate your use of Optmeal's "Services", you can notify us by email or phone. </p>
          <p>The "services" in these Terms and Conditions refer to any functions, information or products provided by Optmeal's applications or websites or other communication channels, including emails, products, delivery services, orders and offers, etc. </p>
          <p> Optmeal is wholly owned and operated by 滿記凍肉有限公司 (in this privacy policy statement, each is referred to as "Optmeal", "we" or "the Company", as applicable depending on the situation). </p>
          <p>These Terms and Conditions apply to members and visitors. </p>
          <p>As a "Visitor" or "Member" of the Optmeal "Service", the collection, use and sharing of your personal information must comply with the Privacy Policy and its updates. </p>

          <h4>1.2 Members and Visitors</h4>
          <p>When you register an account with Optmeal through any means, you become a member. If you choose not to register to use our Services, you may also be able to use certain features as a guest. </p>
          <h4>1.3 Changes</h4>
          <p>Based on business development considerations, we may change the contents of this Code and Terms from time to time. </p>
          <p>We may modify these Terms and Conditions and the Privacy Policy at any time. If there are major changes, we will notify you through the "Service" or other means to give you the opportunity to read the changes in detail before they take effect. We agree that changes cannot be retroactive. If you do not agree with any changed content, you may close your account. If you continue to use our "Services" after we publish or transmit a notice of changes to the terms, you agree to abide by the updated terms from the effective date. </p>
          <h3 class="mx-1">2. Obligations</h3>
          <h4>2.1. Service applicability</h4>
          <p> Optmeal's Services are not intended for use by anyone under the age of 16. </p>

          <h4>2.2 Your Account</h4>
          <p>You will keep your password confidential. </p>
          <p>You will not share your account with others and will comply with all laws that apply to you and our rules. </p>
          <p>Members are account holders. You agree to: (1) use a strong password and not disclose it to others; (2) not transfer any part of your account (such as contacts). Unless you close your account or report abuse, you are solely responsible for all activity on your account. </p>

          <h4>2.3. Payment</h4>
          <p>When you purchase our Services, you will be subject to payment obligations. We do not store any of your payment information (such as credit card information). You understand that our payment process is performed by a third-party service provider selected by us, and therefore we are not responsible for any improper use or processing of data during the payment process. </p>
          <p>We will provide discounts from time to time and you can enter the discount code we provide when paying to get the discounted price. </p>
          <p>Our "Services" are non-refundable, unless it is due to our technical problems that you cannot use the purchased "Services" normally and we are unable to provide you with "Services" through other channels. </p>
          <p>If you purchase any of our paid "services", you agree to pay the relevant handling fees and taxes, if applicable. If payment is not made, we will terminate the paid "services". </p>
          <p>Depending on your location, you may be charged exchange fees or differences (such as exchange rates). </p>
          <p>You can receive a copy of the invoice in the email registered in your account information. </p>

          <h4>2.4. Notifications and Messages</h4>
          <p>You agree that we may provide you with notifications and information through our website, applications and contact information. If your contact information is out of date, you may miss important notifications. </p>

          <h4>2.5 Share</h4>
          When you share information obtained on our "Services", you agree that the information you share complies with all laws and regulations that apply to you, and we will not have any third-party claims or claims arising from your sharing behavior. responsible for the consequences.

          <h3 class="mx-1">3. Rights and Restrictions</h3>
          <h4>3.1. Permissions You Grant Us</h4>
          <p>You own all content, order records, and personal information you provide, but you also grant us non-exclusive permission to use this information. </p>
          <p>We will use the content, product preferences and personal information you provide in our other related services. (For example, push your most suitable products)</p>
          <p>According to the Terms of Use, the content and information you transmit or publish on the "Service" belongs to you, and you only grant us and our affiliates (affiliates are those controlled by us, controlled by us, or jointly with us) Controlled Entity) the following non-exclusive license:</p>
          <p>The worldwide, transferable and sublicensable right to use, copy, modify, distribute, publish and process the information and content you provide through our Services without further obtaining your consent. Agree, without notification and/or compensation. These rights are limited in the following ways:</p>
          <ol>
            <li>Information that has been sent to us through the use of our Services.</li>
            <li>We will not include this content (including sponsored content) in products/platforms/advertising or third-party services provided to others without your consent.</li>
            <li>We may edit or remove the order content you submitted (such as product shortage, removal from shelves, etc.).</li>
          </ol>
          <p>You and we agree that if content contains personal data, it must comply with our privacy policy. </p>
          <p>You and we agree that we may access, store, process and use any information and personal data you provide in accordance with the Privacy Policy. </p>
          <p>By sending suggestions or other feedback related to our Services, you agree that we may use and share your comments (but not necessarily) for any purpose without paying any compensation. </p>
          <p>Please make sure you only provide information and content that you are authorized to share, and that the information in your Optmeal profile is accurate. </p>
          <p>You agree to only provide content or information that is not illegal and does not violate the rights of others, including intellectual property rights. You also agree that your profile information is true. Optmeal may need to remove some content in some countries to comply with local laws. </p>

          <h4>3.2. Service availability</h4>
          <p>We may charge fees or discontinue any Services or modify prices in the future. </p>
          <p>We may change, discontinue or discontinue any of our Services. We may also revise prices with effect in the future after reasonable notice period as permitted by law. </p>
          <p>We do not guarantee that any information and content you post will be stored or continuously displayed. Optmeal non-storage service. Except to the extent required by applicable law and our Privacy Policy, you agree that we have no obligation to store, retain or provide you with copies of any content or information provided by you or others. </p>

          <h4>3.3 Limitations</h4>
          <p>We reserve the right to limit how you establish relationships and interact with us on the Service. </p>

          <h4>3.4 Intellectual property rights</h4>
          <p>We will provide notices regarding Optmeal's intellectual property rights. </p>
          <p>We retain all intellectual property rights in the Service. Trademarks and logos used in the Service may be the trademarks of their respective owners. Optmeal, its logo and other related trademarks, service icons, images, and logos used in the Service are our trademarks or registered trademarks. </p>

          <h3 class="mx-1">4. Disclaimer and Limitation of Liability</h3>
          <h4>4.1. No guarantee</h4>
          <p>We are not legally responsible for the quality, safety or reliability of any Service. </p>
          <p>We and our affiliates do not represent or guarantee any such services, including any representation that such services will be uninterrupted or error-free, and provide such services (including content) on an "as is" and "as available" basis. and information). To the full extent permitted by applicable law, we and our affiliates disclaim any implied or statutory warranties, including any implied warranties of title, accuracy of information, non-infringement, merchantability or fitness for a particular purpose. </p>

          <h4>4.2. Disclaimer</h4>
          <p>The following are any limitations on any legal liability we may have. </p>
          <p>To the full extent permitted by law (and unless we enter into another agreement that replaces these Codes and Terms), we, including our affiliates, are not responsible for any loss of profits or business opportunities or reputational damage (such as specific Offensive or defamatory statements), loss of data (such as downtime or loss of use or alteration of information or content), or any indirect, incidental, consequential, special or punitive damages. </p>

          <h4>4.3 Basis of Agreement; Exceptions</h4>
          The limitations of liability set forth in Section 4 are the basis of the agreement between you and us and apply to all claims of liability (such as warranty, tort, negligence, contract, law) whether or not we or an affiliate were informed of the occurrence of such damages possibility, regardless of whether these remedial measures achieve the necessary purpose.

          <h3 class="mx-1">5. Governing law and dispute resolution</h3>
          Although rare, if a legal dispute arises, you and we agree to resolve it in the courts of the Hong Kong Special Administrative Region, China (subject to the laws of the Hong Kong Special Administrative Region).
          The terms of this section will not deprive you of any mandatory binding consumer protections under the laws and regulations of your country of residence. You can still enjoy these protections in the "services" we provide to you.

          <h3 class="mx-1">6. General Terms</h3>
          <p>The following are some important details about this Code and Terms. </p>
          <p>If a court with jurisdiction over these Codes and Terms finds that any part of these Codes and Terms is unenforceable, both parties agree that the court should modify the terms in a way that retains the original purpose and can be performed. If the court is unable to perform, the parties agree to ask the court to remove the unenforceable portion and fulfill the remaining provisions of the Code and Terms. </p>
          <p>This Code and Terms (including other terms we provide when you use the "Service") are the only terms related to the Service and binding on both parties, and supersede all earlier "Service" agreements. </p>
          <p>If we do not take legal action for violations of the Code and Terms, it does not mean that we have waived our right to perform. </p>

          <h3 class="mx-1">7. What Optmeal can and cannot do</h3>
          <h4>7.1. Can do</h4>
          <p> Optmeal is a sales platform for healthy and high-quality protein products. This list regulates what you can and cannot do when using our Services. </p>
          <p>You agree that you will:</p>
          <ol>
            <li>Comply with all applicable laws, including but not limited to privacy laws, intellectual property laws, anti-spam laws, export administration regulations, tax laws, and regulatory requirements;</li>
            <li>Provide correct information and update it in a timely manner;</li>
            <li>Use your real name on your profile; and</li>
            <li>Use the Service in the correct manner, especially when making purchases.</li>
          </ol>

          <h4>7.2 Can’t do</h4>
          <p>You agree that you will not:</p>
          <ol>
            <li>Create a false identity on Optmeal, misrepresent your identity, create a member profile that is not your own (not a real person), or use or attempt to use another person's account;</li>
            <li>Develop, support or use software, devices, scripts, automated programs or other means or processes (including web crawlers, browser plug-ins and add-ons, or any other technology) to crawl the Service or otherwise Methods to copy profiles and other information from the Service;</li>
            <li>Override any security features, or circumvent or circumvent any access controls or usage restrictions on the Service (such as searching for keywords in capital letters or browsing profiles);</li>
            <li>Copy, use, disclose or distribute any information obtained from the Service (whether directly or indirectly through a third party (such as a search engine)) without our consent;</li>
            <li>Disclose information that you disclosed without your consent (such as another person's confidential information);</li>
            <li>Violate the intellectual property rights of others, including copyrights, patents, trademarks, trade secrets or other proprietary rights.</li>
            <li>Violate our intellectual property rights or other rights, including but not limited to (i) copying or distributing our learning videos or other materials, or (ii) copying or distributing our technology, except for technology released under an open source license. This limit; (iii) use the word "Optmeal" or the logo in any business name, email or website;</li>
            <li>Reverse engineer, decompile, interpret, crack or otherwise attempt to obtain the source code of the Service or any related technology that is not open source;</li>
            <li>Imply or expressly imply that you are an Optmeal affiliate or that you are endorsed by Optmeal (e.g., indicating that you are an Optmeal-approved consultant) without our consent;</li>
            <li>Lease, mortgage, trade, sell/resell the Services or related information or access rights thereto without our consent, or otherwise use the Services or related information or access rights to earn income ;</li>
            <li>Use automated programs or other automated methods to use the Service, add or download contacts, send or redirect messages;</li>
            <li>Monitor the availability, performance or functionality of the Service for any competitive purposes;</li>
            <li>Carry out "framing", "mirroring" or other actions that simulate the appearance or functionality of the "Service";</li>
            <li>Overlay or otherwise modify the Service or its appearance (such as by inserting elements into the Service, or removing, obscuring or obscuring advertisements contained in the Service);</li>
            <li>Interfering with the operation of the Service or creating an unreasonable load (e.g. spam, denial of service attacks, viruses, game algorithms); and/or</li>
          </ol>

          <h3 class="mx-1">8. Content-related complaints</h3>
          <p>Complaint contact information is listed below. </p>

          <h3 class="mx-1">9. How to contact us</h3>
          <div class="mx-1">Email: info@Optmeal.com</div>
          <div class="mx-1">Tel/Whatsapp: 65657682</div>
        </template>
      </v-col>
    </v-row>
  </v-container>
</template>

<script>
export default {
  name : "TermsAndConditions",
  head () {
    return {
      title : this.$t('termsAndConditions') + ' Optmeal',
    }
  },
}
</script>

<style scoped lang="scss">
h3 {
  margin-top: 35px;
}

h4 {
  margin-top: 20px;
}

p {
  margin: 0 0 8px 5px;
}

</style>
